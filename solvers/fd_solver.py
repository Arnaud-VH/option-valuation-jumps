...
def solve(model: JumpDiffusionModel,K, T, r, M=64, N=20, x_star=4.0):
   """
   ...
   Parameters:
   model   : JumpDiffusionModel instance (Merton or Kou model)
   K       : Strike price
   ...
   for i in range(1, n - 1):
      ...
      C[i, i]   = 2.0 * alpha + (r + model.lam) * k  # Main diagonal
   ...
   for i in range(1, n - 1):
      for j in range(n):
         dy = X[j] - X[i]
         trapezoid = -k * h * model.lam * model.jump_density(dy)
   ...
   for m in range(1, q):
      ...
      for i in range(1, n - 1):
         ...
         jump_tail = k * model.lam * model.tail_correction(tau_m, X[i], x_star, K, r)
